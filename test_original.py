...
for index, row in input.iterrows():
   ...
   for respuesta, esperada, key_metrica in zip(respuestas.values(), list(row[1:]), metricas):
        if respuesta == "" and esperada == "":
            ...
            if key_metrica in ["DIMENSIONES", "DIRECCION", "FOT"]:
                if key_metrica =="FOT":
                    correcta = sorted(respuesta.split(". ")) == sorted(esperada.split(". "))
                elif key_metrica == "NOMBRE_BARRIO":
                    correcta = respuesta.lower() == esperada.lower() or ("Barrio "+respuesta).lower() == esperada.lower() or ("Barrio Cerrado "+respuesta).lower() == esperada.lower() or ("Barrio "+esperada).lower() == respuesta.lower() or ("Barrio Cerrado "+esperada).lower() == respuesta.lower()
                else:
